...
let finalCard = document.querySelector(".final");
function gotView () {
got.houses.forEach((i, index) => {
    ...
    a.addEventListener("click", (event) => houseMembers(got, event));
});
}
...
{/* <div class="newCard">
      <div class="people">
          <div class="flex">
            <img src="http://cheatsheetworld.com/img/entertainment/game-of-thrones/game-of-thrones-khal-drogo.jpg" alt="">
            <h4>Eddard "Ned" Stark</h4>
          </div>
          <p>Lord of Winterfell - Warden of the North - Hand of the King - Married to Catelyn (Tully) Stark</p>
      </div>
    </div> */}
function houseMembers(got, event) {
  Array.from(ul.children).forEach(person => person.children[0].classList.remove("highlight"));

  // for(key in ul.children) {
  //   console.log(key);
  //   key.firstElementChild.classList.remove("highlight");
  //   // .classList.remove("hightlight")
  // }
  event.target.classList.add("highlight");
  //console.dir(event.target.parentNode.dataset);
  var inputId = event.target.parentNode.dataset.id;
  //console.log(inputId);
  got.houses.forEach( (house,index) => {
        ...
        if(index == inputId) {
            ...
            house.people.forEach(person => {  
              ...
              let outerSide = document.createElement("div");
              outerSide.classList.add("newCard");
              let innerSide = document.createElement("div");
              innerSide.classList.add("people");
              ...
              flex1.append(img, heading);
              innerSide.append(flex1, para);
              outerSide.append(innerSide);
              finalCard.append(outerSide);
